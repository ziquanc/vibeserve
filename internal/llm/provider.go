...
`)
	...
	b.WriteString("## API Route Design\n\n")
	b.WriteString("Think about WHO uses each route and WHAT they need. Design routes by user role and use case, not just by table.\n\n")
	b.WriteString("For each role in the system, ask:\n")
	b.WriteString("- What can they CREATE? (e.g. student creates test attempts, admin creates questions)\n")
	b.WriteString("- What can they READ? (e.g. student sees own data, admin sees all data)\n")
	b.WriteString("- What can they UPDATE? (e.g. student updates profile, admin updates any user)\n")
	b.WriteString("- What can they DELETE? (e.g. only admin deletes content)\n")
	b.WriteString("- What ACTIONS can they perform? (e.g. student submits test, admin publishes article)\n")
	b.WriteString("- What VIEWS do they need? (e.g. student dashboard, admin analytics)\n\n")
	b.WriteString("Route naming conventions:\n")
	b.WriteString("- Public: GET /posts, GET /posts/:id (no auth needed)\n")
	b.WriteString("- Authenticated user: GET /me, PUT /me, GET /my/orders\n")
	b.WriteString("- Admin: /admin/users, /admin/orders, /admin/dashboard\n")
	b.WriteString("- Actions: POST /orders/:id/approve, POST /posts/:id/publish\n")
	b.WriteString("- Relationships: GET /users/:id/posts, GET /posts/:id/comments\n")
	b.WriteString("- Computed: GET /analytics/overview, GET /reports/sales\n\n")

	b.WriteString("## Route Coverage\n\n")
	b.WriteString("Every MAIN entity table needs routes. But NOT every table needs direct CRUD:\n\n")
	b.WriteString("Tables that NEED direct routes (main entities):\n")
	b.WriteString("- users, posts, products, orders, questions — these are core business objects\n")
	b.WriteString("- Full CRUD: list, get by ID, create, update, delete\n\n")
	b.WriteString("Tables accessed THROUGH parent routes (no direct CRUD needed):\n")
	b.WriteString("- Junction tables (post_tags, user_roles) — managed via parent: POST /posts/:id/tags\n")
	b.WriteString("- Child records (test_answers, order_items) — managed via parent: POST /tests/:id/answer\n")
	b.WriteString("- Computed/tracking tables (topic_mastery, readiness_scores) — read via analytics: GET /student/analytics\n\n")
	b.WriteString("For the 'users' table, ALWAYS include:\n")
	b.WriteString("- GET /me or /users/me — current user profile\n")
	b.WriteString("- PUT /me or /users/me — update own profile\n")
